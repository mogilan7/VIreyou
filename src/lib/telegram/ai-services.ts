...
Твоя задача — предложить 1-2 конкретных варианта приема пищи, чтобы ИДЕАЛЬНО закрыть остаток нормы по КБЖУ с учетом параметров (пол: ${userProfile.gender}, возраст: ${userProfile.age}, вес: ${userProfile.weight} кг, активность: ${userProfile.activity_level}).

КРИТИЧЕСКИ ВАЖНО:
1. Выявление дефицитов: ТЫ ОБЯЗАН вывести четкий список с микронутриентами (витаминами и минералами), показав сколько получено и сколько осталось до нормы (в мг/мкг). Не используй общие фразы — нужны конкретные цифры!
2. Текущее время: У пользователя сейчас ${currentTimeStr}. Если время позднее (после 21:00), строго предостерегай от плотного приема пищи, чтобы не нарушать качество сна и циркадные ритмы. В этом случае предложи максимум очень легкий перекус (например, травяной чай, кефир) или посоветуй оставить дефицит КБЖУ на завтрашний день.
3. Форматирование: Не используй Markdown-разметку (никаких звездочек **). Используй только эмодзи для красоты.

Ответ должен быть в формате ободряющего сообщения для мессенджера (Telegram).
Отвечай на ${lang === 'en' ? 'английском' : 'русском'} языке.
Сразу к делу: чего в цифрах не хватает и что конкретно съесть.
`;
