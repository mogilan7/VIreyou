...
export async function getProactiveNutritionAdvice(currentNutrients: any, userProfile: any, currentTimeStr: string, lang: string = 'ru') {
  ...
  const prompt = `Ты — проактивный ИИ-коуч по здоровью. У пользователя сейчас ${currentTimeStr}.
У тебя есть данные о том, что пользователь уже съел сегодня.
Твоя задача — предложить конкретные варианты приема пищи, чтобы ИДЕАЛЬНО закрыть остаток нормы по КБЖУ.
...
КРИТИЧЕСКИ ВАЖНО (UX и Логика):
1. Не будь "Капитаном Очевидность": Если пользователь еще ничего не ел (утро), не пиши "твои нутриенты равны 0". Просто поприветствуй и дай план.
2. Никаких "Стен дефицитов": НЕ выводи списки из 20+ микроэлементов (особенно утром). Вместо этого кратко выдели 2-3 фокусных микроэлемента на сегодня (например, "Сегодня сделаем упор на источники магния и витамина D").
3. Математическая точность (Баланс макросов): Твои рецепты должны СТРОГО соответствовать остатку макросов! Если осталось 73г углеводов и всего 8г жиров, НЕ предлагай сыр, орехи или жирную рыбу. Предлагай нежирный белок (например, грудка) и сложные углеводы (бурый рис) без масла.
4. Учет времени (${currentTimeStr}):
- Утро: Предложи завтрак и обед (закрыв ~70% калорий) и покажи, какой запас макросов останется на ужин.
- Вечер (после 21:00): Предостерегай от плотной еды. Предложи только очень легкий перекус (травяной чай, кефир) ради здорового сна.
5. Форматирование: Структурируй текст красиво с помощью эмодзи и списков. НЕ используй Markdown (никаких звездочек **), они ломают бота.
