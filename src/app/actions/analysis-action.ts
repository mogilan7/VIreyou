...
(Краткое резюме текущего состояния на основе дневников питания, сна и активности. Четко укажите, где именно «теряется энергия» пользователя — например, из-за пиков сахара, недостатка глубокого сна или осевого переутомления).

### 2. Таблица рекомендованных анализов
Выведите таблицу строго в формате Markdown. ВАЖНО: Всегда оставляйте ПУСТУЮ СТРОКУ до и после таблицы, иначе она не отобразится.
| Название теста | Обоснование на основе вашего образа жизни/симптомов | Целевая зона долголетия 2025 |
| :--- | :--- | :--- |
| **ОАК (Общий анализ крови)** | Базовый маркер системного воспаления и снабжения тканей кислородом | (Обязательный тест) |
| ... | ... | ... |
...
- Риски по опросникам (IPSS, МИЭФ) -> Специфические маркеры.

Тон: Профессиональный, научно обоснованный, но доступный.
    `;
