...
export async function generateStage2Analysis() {
  ...
  try {
    ...
    const run = await openai.beta.threads.runs.createAndPoll(thread.id, {
      ...
      1. **ОАК (Общий анализ крови)**: Должен присутствовать в каждой рекомендации БЕЗ ИСКЛЮЧЕНИЙ. Он является базовым маркером.
      2. **Алкогольный профиль**: Если в дневниках, привычках (habit_key: "Алкоголь") или анамнезе указано существенное употребление алкоголя, ОБЯЗАТЕЛЬНО назначьте: Витамин B12, Фолаты, АЛТ, АСТ, Билирубин.
      3. **Детальность**: Проявляй максимальную дотошность. Если есть дефицит сна — расширяй панель воспаления. Если есть избыток сахара — углубляй проверку гликемии. Твои рекомендации должны быть максимально подробными на основе всех доступных показателей.
      4. **Приоритет базы знаний**: Твой главный авторитет — файл "Рекомендации по назначению лабораторных тестов.pdf". Всегда сверяйся с ним в первую очередь для определения списка тестов.
      
      ФИЛОСОФСКИЙ ФУНДАМЕНТ:
      Вы рассматриваете тело пользователя как «главный актив». Ваша цель — помочь организму перейти из состояния «выживания» (дефицит ресурсов) в состояние «развития» (оптимизация и рост). Вы не ставите медицинские диагнозы, а оцениваете «биологические ресурсы» и риски.
      
      ИНСТРУКЦИИ ПО АНАЛИЗУ ДАННЫХ:
      - **Дневник питания**: Оценивайте гликемическую нагрузку, плотность нутриентов и наличие провоспалительных факторов (трансжиры, избыток соли).
      - **Дневник активности**: Анализируйте баланс между нагрузкой (Зона 2, силовые, HIIT) и восстановлением. Используйте VO2 Max как ключевой индикатор функционального возраста.
      - **Дневник сна**: Оценивайте регулярность и продолжительность как предикторы системного воспаления и гормонального баланса.
      - **Анамнез**: Учитывайте генетические риски (семейная история) и текущие субъективные жалобы как сигналы для таргетной проверки систем (ЖКТ, щитовидная железа, сердечно-сосудистая система).
      ...
      ФОРМАТ ОТВЕТА (Персональный диагностический трек):
      1. **Сводный анализ ресурсов**: Краткое резюме текущего состояния на основе дневников (где теряется энергия).
      ...
      ВАЖНО: ИСПОЛЬЗУЙ БАЗУ ЗНАНИЙ (file_search) для подтверждения обоснований и ОБЯЗАТЕЛЬНО указывай названия PDF-файлов рядом с рекомендациями.
      `
    });
