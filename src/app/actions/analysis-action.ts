...
export async function generateStage2Analysis() {
  ...
  try {
    ...
    const systemInstruction = `
      ...
      1. **БЕЗ РЕКОМЕНДАЦИЙ ПО ОБРАЗУ ЖИЗНИ**: На данном этапе твоя единственная задача — подобрать правильные тесты. **Запрещено** давать советы по питанию, тренировкам, приему БАДов или изменению привычек.
      ...
      ИНСТРУКЦИИ ПО АНАЛИЗУ (ТОЛЬКО ДЛЯ ОБОСНОВАНИЯ ТЕСТОВ):
      - **Дневник питания**: Оценивайте риски (гликемическая нагрузка, наличие провоспалительных факторов) для назначения соответствующих панелей.
      - **Дневник активности**: Анализируйте баланс нагрузки и восстановления для выбора маркеров переутомления и системного стресса.
      - **Дневник сна**: Оценивайте его как предиктор системного воспаления для назначения маркеров inflammaging.
      
      АЛГОРИТМ ФОРМИРОВАНИЯ ДИАГНОСТИЧЕСКОГО ТРЕКА:
      - **Шаг 1: Идентификация метаболического контекста**. Определение необходимости проверки углеводного обмена (HbA1c, инсулин).
      - **Шаг 2: Оценка воспалительного статуса**. Назначение hs-CRP и других маркеров при жалобах или нарушениях режима.
      - **Шаг 3: Сердечно-сосудистая стратификация**. Предложение тестов на ApoB, Lp(a) и липидный профиль при наличии рисков.
      - **Шаг 4: Нуритивная панель**. Определение дефицитов (D, B12, ферритин, магний) для проверки.
      - **Шаг 5: Подготовка к тестам**. Инструкции по правилам сдачи.
      
      ФОРМАТ ОТВЕТА (Персональный диагностический трек):
      1. **Сводный анализ ресурсов (диагностическое обоснование)**: Краткое резюме текущего состояния (где теряется энергия) исключительно как аргументация для выбора анализов.
    ...
    const model = getGeminiModel("gemini-2.5-flash", 0.2, false, systemInstruction);
